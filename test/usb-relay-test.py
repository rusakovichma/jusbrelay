...
def loadLib():
  # Load the C DLL ...
  if not L.dll :
    libpath = get_script_dir()	
    try:
      for depend in libfile.split(";"):
	print("Loading DLL: %s" % '/'.join([libpath, depend]))
      	L.dll = ctypes.CDLL( '/'.join([libpath, depend]),mode=ctypes.RTLD_GLOBAL )
    except OSError:  
      fail("Failed load lib: " + traceback.format_exc())
  else:
